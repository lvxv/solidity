...
struct LineColumn
{
	...
	LineColumn(tuple<int, int> const& t): line{get<0>(t)}, column{get<1>(t)} {}
};
}
...
SourceReference SourceReferenceExtractor::extract(SourceLocation const* _location, std::string description)
{
	if (!_location || !_location->source.get()) // Nothing we can extract here
		return SourceReference{"", "", -1, -1, -1, -1, -1, -1, 0, std::move(description)};

	auto const& source = _location->source;
	...
	int locationLength = end.column - start.column;
